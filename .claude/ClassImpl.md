...
`Double`, `Void`; `String` (params + returns); `[UInt8]` (params + returns);
optionals of primitives & `String` (params + returns).
...
| Init / factory | Body yes (`initialize(to: T(...))`), shape no — write a custom thunk-allocating emitter. Precedent: `optionalReturningThunk` / `arrayReturningThunk` in `+SwiftThunkPrinting.swift`. |
...
- KN optional/array thunks: `+SwiftThunkPrinting.swift:205-206`, `:301-302`,
  `:376,:380`.
