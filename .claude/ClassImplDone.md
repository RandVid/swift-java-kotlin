...
`String`, and custom imported class/struct types — across constructors,
instance/static methods, and property accessors. Enums, protocols, generics,
subscripts, async, and throwing members are skipped with a comment.
...
| `…/KotlinNativeSwift2KotlinGenerator+Classes.swift` (new) | `memberIsEmittable` (CdeclLowering gate); `printKotlinClass`, `renderConstructor`/`renderMethod`/`renderProperties`, `kotlinParamAndArg` |
...
| `…/KotlinNativeSwift2KotlinGenerator+SwiftThunkPrinting.swift` | member gate, `makeThunkDecl`, `nominalAllocatingThunk`, `destroyThunk`; `memberCFunction`; default/object-return paths via `loweredCdeclForThunk`; global var thunk emission |
| `Tests/JExtractSwiftTests/KotlinNative/KotlinNativeClassTests.swift` (new) | class/struct generation tests |
| `Tests/JExtractSwiftTests/KotlinNative/KotlinNativeTopLevelFunctionsTests.swift` | +global variable tests (`globalVar_readWrite_kotlin`, `globalVar_readOnly_kotlin`, `globalVar_string_kotlin`, `globalVar_customObject_kotlin`, `globalVar_readWrite_swiftThunk`) |
| `Samples/KotlinNativeSampleApp/Sources/SimpleSwiftLib/SimpleSwiftLib.swift` | `Counter` class, `combine`, init/deinit counters, `globalScore`/`appVersion` globals |
| `Samples/KotlinNativeSampleApp/src/macosArm64Test/kotlin/SimpleSwiftLibTest.kt` | functional + memory + GC cleaner + global var runtime tests |
| `Samples/KotlinNativeSampleApp/build.gradle.kts` | `inputs.file(swiftJavaTool)` + `SwiftKitKN` dependency |
| `CLAUDE.md` | doc refresh |

### Verification performed
- `swift test` → **KotlinNative** unit tests pass (top-level + class, incl. global
  variable tests).
- `Samples/KotlinNativeSampleApp/ci-validate.sh` → **BUILD SUCCESSFUL** (requires a
  fresh `ci-validate.sh` run to regenerate bindings).
...
**Types supported:** all primitives, `Bool`, `Float`, `Double`, `String`, and custom
class/struct objects (see the unification below).
...
### Custom-object global variables + `renderPropertyAccessorLines` unification

Global variables support the same types class properties support — primitives,
`String`, and custom class/struct objects.
...
- `NativeGlobalVar` struct: `setterCallArg: String` → `setterPA` (full
  `kotlinParamAndArg` result) carrying the setter value's call expression.

**Tests added:** `globalVar_customObject_kotlin` unit test; `shared` sample global.
