...
package class KotlinNativeSwift2KotlinGenerator {
  ...
  func generate() throws {
    ...
    // No cinterop C header is emitted: the Swift `@_cdecl` thunks are bound directly
    // as `@ImportedBridge` externals inside the Kotlin wrapper (see
    // `printImportedBridgeExterns`), so there is no `.def`/klib step.
  ...
  func printObjectWrapHelper(_ printer: inout CodePrinter) {
    printer.print("private inline fun wrapSwiftObject(make: () -> NativePtr): NSObject =")
    printer.print("  autoreleasepool { interpretObjCPointer(make()) }")
  }
  ...
  func printKotlinModuleFile(_ printer: inout CodePrinter) {
    ...
    printer.print("// Calls Swift @_cdecl C thunks directly via @ImportedBridge (no cinterop klib)")
    // `@ImportedBridge` / `NativePtr` are in kotlin.native.internal. Its opt-in marker
    // `InternalForKotlinNative` is itself `internal`, so it cannot be named in a source
    // `@OptIn(...)`; the consuming Kotlin/Native module must pass the compiler flag
    // `-opt-in=kotlin.native.internal.InternalForKotlinNative` (plus
    // `kotlinx.cinterop.ExperimentalForeignApi` for the marshalling helpers).
    printer.print("package \(kotlinPackage)\n")
    // The Swift `@_cdecl` thunks are bound directly as `@ImportedBridge` externals
    // (see the extern block below) instead of through a generated cinterop package.
    printer.print("import kotlin.native.internal.NativePtr")
    printer.print("import kotlin.native.internal.ImportedBridge")
    ...
    // Direct bindings to the Swift @_cdecl thunk symbols (replaces the cinterop klib).
    printImportedBridgeExterns(&printer)
  }
  ...
  func inoutRefSource(_ m: InoutMarshal) -> String {
    ...
    // The box cell is a `COpaquePointerVar`; `__ptr()` yields a `NativePtr`, so wrap it.
    case .boxed:  return "interpretCPointer<CPointed>(\(m.paramName).unsafeValue.__ptr())"
    }
  }
  ...
  func inoutCellReadBack(_ m: InoutMarshal) -> String {
    ...
    case .boxed(_, let wrapperType):
      return "\(wrapperType)(wrapSwiftObject { \(m.cellVar).value!!.rawValue })"
    }
  }
  ...
  func returnBodyLines(callExpr: String, ret: KotlinType, indent: String, finalPrefix: String) -> [String] {
    ...
    case .string:
      // Swift returns the String as an autoreleased NSString box (void*); read it back
      // into a Kotlin String inside an `autoreleasepool` so the thunk's
      // `passRetained(...).autorelease()` +1 is balanced (same pattern as objects,
      // `printObjectWrapHelper`). `interpretObjCPointer<String>` copies the NSString
      // contents into a Kotlin String, after which the NSString can be released.
      return [
        "\(indent)\(finalPrefix)autoreleasepool { interpretObjCPointer<String>(\(callExpr)) }",
      ]
  ...
  // MARK: - @ImportedBridge externals

  /// Emit `@ImportedBridge` external-function declarations that bind each Swift
  /// `@_cdecl` thunk symbol directly into Kotlin/Native — replacing the cinterop
  /// `.def`/klib. The declaration set is derived from the same `CFunction`s the C
  /// header used to print, so it stays in lockstep with the Swift thunk file and the
  /// Kotlin wrappers: every symbol the wrappers call is declared here exactly once.
  func printImportedBridgeExterns(_ printer: inout CodePrinter) {
    printer.print("// ---- @ImportedBridge externals: direct bindings to the Swift @_cdecl thunks ----")
    for resolved in resolvedFunctions() {
      guard case .emit(let fn) = resolved else { continue }
      printExtern(&printer, fn.cFunction)
    }

    // Global variable getter/setter thunks.
    for decl in emittableGlobalVarAccessors {
      if let cFunction = try? memberCFunction(for: decl) {
        printExtern(&printer, cFunction)
      }
    }

    // Nominal-type members, struct `copy`, and per-type `_destroy` thunks.
    for typeName in analysis.importedTypes.keys.sorted() {
      guard let nominal = analysis.importedTypes[typeName] else { continue }
  ...
  /// Print one `@ImportedBridge("sym") external fun sym(p0: T0, …): R` declaration.
  /// Parameter names are synthesized (`p0`, `p1`, …) since only the symbol string
  /// matters for linkage.
  private func printExtern(_ printer: inout CodePrinter, _ cFunction: CFunction) {
    let params = cFunction.parameters.enumerated()
      .map { (i, p) in "p\(i): \(kotlinExternType(p.type))" }
      .joined(separator: ", ")
    printer.print("@ImportedBridge(\"\(cFunction.name)\")")
    printer.print("external fun \(cFunction.name)(\(params)): \(kotlinExternType(cFunction.resultType))")
  }

  /// Map a lowered C type to the Kotlin type used in an `@ImportedBridge` extern
  /// signature: primitives map 1:1, and every pointer (object/String box, `inout`
  /// cell, out-parameter) crosses as `NativePtr`.
  func kotlinExternType(_ c: CType) -> String {
    switch c {
    case .void:
      return "Unit"
    case .integral(let i):
    ...
    case .pointer:
      return "NativePtr"
    case .qualified(_, _, let inner):
      return kotlinExternType(inner)
    case .tag, .function:
      return "NativePtr"
    }
  }
  ...
  func kotlinParamAndArg(
    ...
    switch kt {
    case .string:
      // Pass the bridged NSString's +0 pointer; the thunk raises it back to a String.
      return (param, ["\(name).objcPtr()"])
    case .object:
