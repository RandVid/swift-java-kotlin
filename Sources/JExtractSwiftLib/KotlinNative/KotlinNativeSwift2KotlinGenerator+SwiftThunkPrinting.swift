...
extension KotlinNativeSwift2KotlinGenerator {
  ...
  package func writeSwiftThunkSources(printer: inout CodePrinter) throws {
    ...
    // Foundation supplies `NSString`, used to bridge `String` params/returns as
    // ObjC objects across the @ImportedBridge/NativePtr transport.
  ...
  func loweredCdeclForThunk(
    ...
    // KN String ABI: bridge `String` as an ObjC `NSString` object (a `void*`), not a
    // C string. This matches the @ImportedBridge/NativePtr transport (every pointer is
    // a `NativePtr`) and needs no string-runtime helper. A String parameter arrives as
    // a +0 pointer to a bridged NSString (Kotlin `String.objcPtr()`), raised with
    // `Unmanaged<NSString>.fromOpaque(...).takeUnretainedValue() as String`. A String
    // result is returned as an autoreleased NSString box (`Unmanaged.passRetained(...
    // as NSString).autorelease().toOpaque()`) that the Kotlin side turns back into a
    // String with `interpretObjCPointer<String>`.
    let knownTypes = SwiftKnownTypes(symbolTable: symbolTable)
    for (i, param) in decl.functionSignature.parameters.enumerated() {
      guard case .string? = swiftTypeToKotlin(param.type) else { continue }
      lowered.parameters[i].cdeclParameters = [
        SwiftParameter(
          convention: .byValue,
          parameterName: lowered.parameters[i].cdeclParameters.first?.parameterName,
          type: knownTypes.unsafeRawPointer
        )
      ]
      lowered.parameters[i].conversion = knStringRaiseConversion()
    }
    if case .string? = swiftTypeToKotlin(decl.functionSignature.result.type) {
      lowered.result.cdeclResultType = knownTypes.unsafeMutableRawPointer
      lowered.result.conversion = knStringBoxConversion()
      // `cdeclReturnTypeForThunk` is stored at lowering time, so update it too or the
      // thunk keeps the stale `char*` return type. Mirror the throwing/optional rule.
      lowered.cdeclReturnTypeForThunk = lowered.isThrowing
        ? knownTypes.optionalSugar(knownTypes.unsafeMutableRawPointer)
        : knownTypes.unsafeMutableRawPointer
    }
    return lowered
  }

  /// `(Unmanaged<NSString>.fromOpaque(<placeholder>).takeUnretainedValue()) as String`
  /// — raise a `+0` NSString box pointer into a Swift `String` for a String parameter.
  func knStringRaiseConversion() -> ConversionStep {
    let raise: ConversionStep = .member(
      .method(
        base: "Unmanaged<NSString>",
        methodName: "fromOpaque",
        arguments: [LabeledArgument(label: nil, argument: .placeholder)]
      ),
      member: "takeUnretainedValue()"
    )
    return .cast(raise, type: "String")
  }

  /// `Unmanaged.passRetained((<placeholder>) as NSString).autorelease().toOpaque()`
  /// — box a Swift `String` result as an autoreleased NSString `void*` pointer.
  func knStringBoxConversion() -> ConversionStep {
    let passRetained: ConversionStep = .method(
      base: "Unmanaged",
      methodName: "passRetained",
      arguments: [LabeledArgument(label: nil, argument: .cast(.placeholder, type: "NSString"))]
    )
    return .member(.member(passRetained, member: "autorelease()"), member: "toOpaque()")
  }
