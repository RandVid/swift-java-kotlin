...
- **Kotlin/Native** (`kotlinNative`) - NEW: Generates Kotlin/Native source that calls the Swift `@_cdecl` C thunks **directly via cinterop, with no JVM / Java FFM layer**. The most actively developed mode — supports a broad range of primitive, String, `[UInt8]`, and optional types.
...
of truth as FFM's Java `FunctionDescriptor`), except for array/optional returns where FFM's
callback/no-nullable-scalar limitations force a custom `CFunction` (see `.claude/ArrayImpl.md`,
`.claude/OptionalImpl.md`).
...
│   └── KotlinNative/       # Kotlin/Native generator (3 files, ACTIVE)
...
│       └── KotlinType.swift                                    # KotlinType enum (primitives, array, optional)
...
└── KotlinNative/KotlinNativeTopLevelFunctionsTests.swift  # 75 Kotlin/Native generation tests
...
**Kotlin Type Mapping** (the `KotlinType` enum in `KotlinNative/KotlinType.swift` models all of these,
including `.array(KotlinType)` and `.optional(KotlinType)`):
...
| `[UInt8]` | `UByteArray` | params + returns (kotlinNative only) |
| `T?` | `T?` | optional primitives, `Bool?`, `Float?`, `String?` (kotlinNative only) |
...
- **`kotlinNative`** — the full table above. String returns, `[UInt8]`/`UByteArray`, and optionals are all supported because the Kotlin/Native generator builds its own ABI rather than delegating to FFM (which has no nullable scalars and uses a callback ABI for arrays). Custom **classes and structs** are bridged as wrapper classes over a Swift-allocated box. Async functions and nested optionals (`Int??`, `[T]?`) are still skipped with a `// Skipped …` comment.
...
- Top-level functions over a broad type set: all signed/unsigned integer widths, `Bool`, `Float`,
  `Double`, `String` (params **and** returns), `[UInt8]`/`UByteArray` (params and returns), and
  optionals (`Int?`, `Int32?`, …, `Bool?`, `Float?`, `Double?`, `String?`) as params and returns.
- **Custom classes & structs** (`KotlinNative/KotlinNativeSwift2KotlinGenerator+Classes.swift`):
  ...
  and other custom types (no optionals/arrays on members; those remain on top-level functions).
...
- **Arrays** — FFM uses a callback ABI (`Linker.upcallStub`); Kotlin/Native's `staticCFunction` has
  no closure context, so arrays use a **heap-pointer + out-count** ABI. `[UInt8]` params are pinned
  with `usePinned { addressOf(0) }`; returns are `memScoped { alloc<LongVar>() … readBytes() }` and
  the caller must `free()` the pointer (`import platform.posix.free`, emitted only when needed).
  See `.claude/ArrayImpl.md`.
- **Optionals** — Java/FFM has no nullable scalars, so `CdeclLowering` throws on optional params/
  returns. The generator strips the optional, lowers the wrapped type, then builds a custom
  `CFunction` returning `UnsafeMutablePointer<T>?` (null = `nil`). `String?` has a dedicated path
  (`stringOptionalAwareThunk`) since `String` is passed as a `char*`. See `.claude/OptionalImpl.md`.
...
- `ArrayImpl.md` — `[UInt8]`/`UByteArray` ABI, the SIGBUS bug from running the SwiftPM plugin in
  default FFM mode, and its fix (declare `"mode": "kotlinNative"` + write Swift thunks/placeholders).
- `OptionalImpl.md` — optional params/returns and `String?`, signature stripping, thunk dispatch order.
- `PRIMITIVE_ARRAY_IMPL.md` — design notes for generalising arrays beyond `[UInt8]` to all primitive
  element types (not yet implemented).

**Known Limitations:**
- Classes & structs are supported; enums, protocols, and generics are not. No collections beyond
  `[UInt8]`.
- Members are limited to primitive/`String`/custom-type parameters and returns (no optional/array
  members; no subscripts; no throwing members that return a custom type). Failable inits are skipped.
...
- Nested optionals (`Int??`) and `Optional<Array>` (`[T]?`) are skipped.
...
`"mode": "kotlinNative"`. Omitting it silently falls back to FFM mode, which emits a callback-ABI
thunk that crashes at runtime with **SIGBUS** (the array out-count pointer is misread as a C
function pointer). See `.claude/ArrayImpl.md` § "The SIGBUS Bug and Its Fix".
...
- `Tests/JExtractSwiftTests/KotlinNative/KotlinNativeTopLevelFunctionsTests.swift` - Kotlin/Native tests (75)
